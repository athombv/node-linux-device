...
class DeviceHandle extends Duplex {
	...
	_writev(chunks, callback) {
		util.callbackify(this._writevPromise).apply(this, arguments);
	}
	
	async _writePromise(chunk, encoding) {
		if(!this.fd) throw new Error('not_open');
		if(!this[kSource].writable) throw new Error('not_writable');
		if(chunk.gap && chunk.repetitions) {
			const chunks = [];
			for(var i = 0; i < chunk.repetitions; i++) {
				chunks.push({chunk});
				if(1 !== chunk.repetitions - 1) {
					chunks.push({chunk: chunk.gap});
				}
			}
			return await this[kSource].handle.writev(chunks); //retain chunks
		} else {
			return await this[kSource].handle.write(chunk); //retain chunk
		}
	}
	
	async _writevPromise(chunks, encoding) {
		if(!this.fd) throw new Error('not_open');
		if(!this[kSource].writable) throw new Error('not_writable');
		return await this[kSource].handle.writev(chunks); //retain chunks
	}
